...
| Kernel compression mode (ZSTD) |   ---&gt; | Mode de compression du noyau (ZSTD). |
...
| IRQ subsystem |   ---&gt; | Sous-système des interruptions (IRQ). |
| Timers subsystem |   ---&gt; | Sous-système des compteurs de temps. |
| BPF subsystem |   ---&gt; | Sous-système BPF (Berkeley Packet Filter). |
| Preemption Model |   ---&gt; | Modèle de préemptibilité (sous-contrôlé par le scheduleur). |
...
| RCU Subsystem |   ---&gt; | Sous-système RCU (Read-Copy-Update). |
...
| Scheduler features |   ---&gt; | Fonctionnalités du planificateur. |
